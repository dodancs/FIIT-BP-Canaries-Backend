...
def CanarySetup(self, configPath = ''):
    data = ConfigData()
...
**************************')
    ...
    i = input('Bind address (0.0.0.0): ')
    data.bindIP = '0.0.0.0' if not i else i

    try:
        i = input('Listen on port (5000): ')
    ...
    skip = 0
    data.defaultPassword = getpass.getpass('Default administrator account password: ')
    if not data.defaultPassword:
        if input('Are you sure you want to use empty password? (y/N): ') in CanaryHelpers().yes:
            data.defaultPassword = ''
            skip = 1
        else:
            data.defaultPassword = getpass.getpass('Default administrator account password: ')
    if not skip:
        i = getpass.getpass('Repeat password: ')
        while i != data.defaultPassword:
    ...
    i = input('Default administrator account username (admin): ')
    data.defaultUser = 'admin' if not i else i
